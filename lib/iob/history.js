...
function calcTempTreatments (inputs, zeroTempDuration) {
	...
	console.error(profile_data);
    var autosens_data = inputs.autosens;
	console.error(autosens_data);
    var tempHistory = [];
    var tempBoluses = [];
    var pumpSuspends = [];
    var pumpResumes = [];
    ...
    for (var i=0; i < pumpHistory.length; i++) {
        ...
        if (current._type == "Bolus") {
            ...
            var temp = {};
            if ((current.duration) && (current.absolute)) {
           ...
           // console.error(tempHistory.length);
                //console.error("AAPS Temp "+temp.id+" "+temp.date+" "+temp.rate+" "+temp.duration+" ");
            } else {
                var previous = tempHistory;
                // console.error(previous.slice(-1));
                //console.error("Aenderung erforderlich "+previous._id+" ");
            }
...
console.error(calcTempTreatments);
